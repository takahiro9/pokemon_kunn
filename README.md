...
- **対戦形式:** `data/config/custom-formats.ts` で定義したカスタムフォーマット（フォーマット ID
  ...
  - **チームプレビュー:** どの 3 匹を選ぶか（相手の見せ札の種族を見た上で、先発順込みで 6 匹から 3 匹選ぶ）も
    学習対象になっている。poke-env の既定実装（`random_teampreview`）はシングルバトルでは常にランダム選出だが、
    自分側エージェントについてはこれを差し替え、既存の交代アクション用ヘッドを再利用して学習する
    （詳しくは [6 章](#6-srcpokeai--コードの概要)、対戦相手側はまだランダム選出のままで未対応タスク参照）
...
| `tests/` | 観測エンコーダ・アクションマスク・チームプレビューの単体テスト（サーバ不要） |
...
2. **観測の作成**（`encoding.encode_battle`）
   毎ターン、poke-env が保持している対戦状況（両チーム・判明している相手の情報・天候/フィールド・技の情報など）を 2386 次元の数値ベクトルに変換し、
   同時に「今選べる行動」を表す 26 次元のマスクも作ります。チームプレビュー中はこの同じ観測・マスクの仕組みを流用し、
   交代アクション（0〜5）を「自分の何番目を選ぶか」に読み替えます（詳しくは 6 章の `env.py` を参照）。
3. **行動選択とデータ収集（ロールアウト）**
...
バトルの状態を固定長（2386 次元）の float32 ベクトルにします。中身は 4 つのブロックに分かれています。

```
[GLOBAL 82] [ポケモン数値 49 × 12体] [ポケモンID 7 × 12体] [技数値 34 × 12体 × 4技]
```
...
- **GLOBAL:** 天候、フィールド、両陣営の場の状態（まきびし等は段階数）、ターン数、メガシンカ可能/使用済み（自分・相手）、残りポケモン数、強制交代中か、交代不可か、チームプレビュー中か、など。
- **ポケモン数値:** 存在フラグ・自分か・場にいるか・ひんしか・HP 割合・判明済みか・メガシンカ済みか・レベル・チームプレビューで選出済みか、状態異常、能力ランク、タイプ、種族値。
...
- `teampreview_action_mask()`: チームプレビュー中のアクションマスク。まだ選んでいない自分のポケモンに対応する交代アクション（0〜5）だけを許可する。
...
- `PokemonEnv`: poke-env の `SinglesEnv` を継承し、観測を `encoding.encode_battle`、報酬を `RewardConfig` に基づく計算に差し替えたもの。
  加えて `action_to_order` / `get_action_mask` をオーバーライドし、`battle.teampreview` 中は交代アクションを
  「チームプレビューでの選出」として扱う。
  - **チームプレビューの学習:** poke-env は VGC（ダブルス）でしかチームプレビューを行動として扱えず、
    シングルバトルでは常に `_EnvPlayer._teampreview` が `random_teampreview` にフォールバックする。
    そこで `PokemonEnv.__init__` で自分側エージェント（`agent1`）の `teampreview` メソッドを差し替え、
    `opponents.run_teampreview()` で 6 匹から 3 匹を（先発順込みで）同期的に決定する。
    使うモデルは self-play プールと同じ仕組みで定期的にディスクから再読込する（`reload_teampreview()`）。
    決定した各選出の観測・行動・対数確率・価値は `reset()` の戻り値の `info["teampreview"]` に載せて
    学習ループへ渡し、`ppo.py` 側でそのエピソードの勝敗が確定した時点で PPO の更新バッチに合流させる
    （モンテカルロ収益、詳しくは下の `ppo.py` の節）。相手側（`agent2`、および self-play の相手として
    使われる `env.opponent`）はここでは変更しておらず、引き続きランダム選出。
...
- `run_teampreview()`: モデルの交代アクション用ヘッドを 3 回（マスクを毎回更新しながら）呼び出し、6 匹から 3 匹を先発順込みで選ぶ。
  `"/team ..."` の指令文字列と、学習用の各選出の観測・行動・対数確率・価値をまとめて返す。
- `PolicyPlayer`: 学習済みモデルで行動を選ぶ poke-env の `Player`。self-play の相手としても評価対象としても同じ推論経路を使います。1 手あたりの推論時間も計測。
  `teampreview()` は `run_teampreview()` を使うので、単体評価（`evaluate.py`）やデプロイ時はチームプレビューの選出も学習済みのものになる。
...
チームプレビューの選出は毎ターンの行動選択ループの外（エピソード開始時に 1 回）で決まるため、通常の
`obs_buf` / `actions` などの固定長バッファには乗らない。代わりに `_PendingTP` で環境ごとに「保留中の選出」を
持ち、`envs.step()` の戻り値の報酬を毎ステップ割引加算していき、そのエピソードが終了した時点（＝勝敗が
決まった時点）で確定した収益を教師信号として `tp_ready` に積む。1 回の更新の最後に、溜まった選出をモンテ
カルロ収益（アドバンテージ ＝ 収益 − 価値、λ=1 の GAE に相当）の遷移として通常の PPO バッチに連結し、
同じ方策損失・価値損失・クリップ処理で一緒に更新する（新しいモデルヘッドは追加していない）。
`snapshot()` で self-play プールに新しいスナップショットを追加するのと同じタイミングで、各並列環境の
チームプレビュー用ポリシーも最新のチェックポイントに再読込する。
...
- 対戦相手側のチームプレビュー学習: 自分側エージェントの選出は学習済み（6 章参照）だが、対戦相手
  （ベースラインおよび `env.opponent` として使われる self-play スナップショット）は `SingleAgentWrapper` が
  VGC 以外ではチームプレビューを行動として扱わないため、引き続き poke-env の既定のランダム選出のまま。
  単体評価・デプロイ用の `PolicyPlayer.teampreview` は学習済みの選出を使う。
